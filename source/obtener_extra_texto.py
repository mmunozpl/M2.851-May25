    ...
    try:
        ...
        for dt in dl.find_all('dt'):
            ...
            if not dd:
                continue
            value = dd.get_text(" ", strip=True)
            if "códigos cpv" in key_text:
                campos["Codigos_CPV"] = value
            elif "cpv (referencia de nomenclatura)" in key_text:
                # lógica para casos anteriores a marzo de 2018
                campos["Codigos_CPV"] = value  # se conserva código + descripción
    except Exception as e:
        print(f"Error obteniendo extra texto de {enlace_html}: {e}")
    return campos
